...
//////////////////////////////////////////////////////////////////////////
DiscretizedArea::DiscretizedArea(Shape2D const& _external, std::set< Shape2D > const& _obstacles) 
	: m_graph(nullptr)
	, m_listGraph(nullptr)
	, m_numberOfValidSquare(-1)
	, m_sinkCoordinate(-1,-1)
...
IDSReal2D Square::agentVertex(int i) const
{
	...
	if(i == 2)
		...
		index = 2;
	Line2D l_line(m_box.corner( index ), m_box.center());
	double l_dist = m_box.corner( index ).distance(m_box.center());
